...
class VirtualJoystick extends Phaser.GameObjects.Container {
    ...
    constructor(scene, x, y, radius = 110) {
        super(scene, x, y);
        this.radius = radius;
...
export class TouchPadDebugScene extends Phaser.Scene {
    ...
    aimVector = new Phaser.Math.Vector2();
    constructor() {
    ...
    setupInput() {
        ...
        this.inputAdapter.on(INPUT_ACTIONS.MOVE, (state) => {
            if (state.type === 'analog')
                this.moveVector.copy(state.vector);
        });
        this.inputAdapter.on(INPUT_ACTIONS.AIM, (state) => {
            if (state.type === 'analog')
                this.aimVector.copy(state.vector);
        });
